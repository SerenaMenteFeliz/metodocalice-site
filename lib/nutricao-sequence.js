...
// cid (contact_id) identifica a visita na PostHog mesmo vindo de um
// aparelho/sessão diferente do quiz original (ver identify() em
// material/index.html, achado 04/08) -- sem isso, clicar o link do
// e-mail em outro dispositivo virava uma pessoa anônima nova, sem
// ligação nenhuma com quem completou o quiz, e o painel não conseguia
// atribuir a visita corretamente.
function materialUrl(result, contactId) {
  const base = 'https://metodocalice.serenamentefeliz.com/material';
  const params = new URLSearchParams();
  if (result) params.set('r', result);
  if (contactId) params.set('cid', contactId);
...
const SEQUENCE = [
  {
    ...
    html: ({ nome, result, unsubscribeUrl, contactId }) => {
      ...
      return wrap(
        ...
        ${button(materialUrl(result, contactId), 'Ler o material')}
  ...
  },
  {
    ...
    html: ({ nome, result, unsubscribeUrl, contactId }) => wrap(
      `
      <p>Oi${nome ? ', ' + nome : ''}.</p>
      ...
      ${button(materialUrl(result, contactId), 'Reler o material')}
      <p>Geovana &middot; Método Cálice</p>
      `,
      unsubscribeUrl
    ),
